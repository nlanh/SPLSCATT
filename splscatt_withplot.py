...
def main():
    ...
    print("\nUnit system")
    print("1. Nuclear units (MeV, fm)")
    print("2. Natural units (ħ = c = 1)")

    unit_system = int(input("Choice = "))
    ...
    if not os.path.exists(filename):
        filename = input("Potential file: ")

    print(f"Using potential: {filename}")

    mu = float(input("Reduced mass mu (amu): "))
    E = float(input("Scattering energy E (MeV): "))
    lmax = int(input("lmax = "))
    ...
    ax1.semilogy(theta_deg, dsdO)

    ax1.set_xlabel(r"$\theta$ (deg)")
    ax1.set_ylabel(
        r"d$\sigma$/d$\Omega$ (fm$^2$/sr)"
    )
